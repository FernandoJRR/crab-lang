...
#[derive(Debug)]
pub struct TypeSymTable {
    pub values: RefCell<HashMap<String, Type>>,
    pub parent: RefCell<Weak<TypeSymTable>>,
    pub children: RefCell<Vec<Rc<TypeSymTable>>>,
}
...
impl TypeSymTable {
    ...
    pub fn set(&self, name: String, val: Type) {
        self.values.borrow_mut().insert(name, val);
    }
...
#[derive(Debug)]
pub struct ScopedTypeSymTable {
    pub current_scope: Rc<TypeSymTable>,
    parent_stack: Vec<Rc<TypeSymTable>>
}

impl ScopedTypeSymTable {
    pub fn new() -> Self {
        Self { current_scope: TypeSymTable::new(), parent_stack: vec![] }
    }

    pub fn push_scope(&mut self) {
        let new_scope = TypeSymTable::new_child(Rc::clone(&self.current_scope));
        self.parent_stack.push(Rc::clone(&self.current_scope));
        self.current_scope = new_scope;
    }
